...
| Fecha | Descripción |
|-------|-------------|
| 2026-07-14 | Mejora ad-hoc (fuera del sprint activo, a pedido explícito del usuario) en Plan de Cuentas + Diario: `plan-cuentas.js` ahora tiene subgrupo completo en las ~70 cuentas base + taxonomía `_SUBGRUPOS_POR_GRUPO`, con reparación automática (`_repararPlanCuentas`) de planes guardados sin subgrupo; nuevas `_inferirSubgrupo`/`_inferirTipoGrupoCuenta` para clasificar cuentas nuevas; `_detectarCuentasNoRegistradas` + `_confirmarYCrearCuentasFaltantes` (con `mostrarConfirm`) integradas en `guardarAsiento()`/`guardarAsientoManual()` de `diario.js` para dar de alta cuentas usadas pero no registradas; botón "🔍 Auditar cuentas faltantes" en Plan de Cuentas para revisar el histórico. `diario.js`: cuentas nuevas (`Materias Primas`, `Préstamos Bancarios CP`, `Previsión Social por Pagar`, `Seguros`, `Gastos de Mantenimiento`, `Comisiones Pagadas`, `Devoluciones y Descuentos`) y reúso de `Intereses Pagados` en vez de una cuenta inexistente; ajuste de ambigüedad "compra de servicio" vs "gasto". Corregido bug `iaTentarGlosa()` inexistente + id `btnIaGlosa` duplicado en `index.html`/`ia.js` (botón IA del tab principal y del modal manual). Cache del service worker bumpeado a `auditor-beta-v2`. Verificado con `verificar.js` (26/26 ✅) y script Playwright ad-hoc (28/28 ✅). |
